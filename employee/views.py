...
#    ws.row(0).write(0, value, xlwt.Style.easyxf(style))
    ...
    ws.write_merge(3, 3, 0, 9, company_contacts,
                   xlwt.Style.easyxf(style))
    style = 'align: wrap on, vert centre, horiz center; font: bold on'
    ws.write_merge(5, 5, 0, 0, 'Date.',
                   xlwt.Style.easyxf(style))
    ws.write_merge(5, 5, 1, 4, f'{datetime.now().strftime("%Y-%m-%d")}',
                   xlwt.Style.easyxf(style))
    ws.write_merge(6, 6, 0, 0, 'Mr./Ms./Mrs.',
                   xlwt.Style.easyxf(style))
    ws.write_merge(6, 6, 1, 5, '',
                   xlwt.Style.easyxf(style))
    ws.write_merge(8, 8, 0, 8, 'This is proof that your name has been added to our staff list.',
                   xlwt.Style.easyxf(style))
    style = 'align: wrap on; font: bold on'
    ws.write_merge(10, 10, 0, 9, 'In this regard, we are please to infrom you that you are one of our recipients and ',
                   xlwt.Style.easyxf(style))
    ws.write_merge(11, 11, 0, 9, 'you are subject to the following conditions:',
                   xlwt.Style.easyxf(style))
    ws.write_merge(13, 13, 1, 3, '1. Staff status',
                   xlwt.Style.easyxf(style))
    ...
    ws.write_merge(20, 20, 1, 3, '2. Time',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(20, 20, 5, 5, 'from',
                   xlwt.Style.easyxf(style_ordinary))
    ...
    ws.write_merge(22, 22, 5, 5, 'Until',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(22, 22, 6, 9, f'{employee.employee.contract_expiration}',
                   xlwt.Style.easyxf('borders: top_color white, bottom_color black, right_color white, left_color white,\
                              left thin, right thin, top thin, bottom thin;'))
    ws.write_merge(23, 23, 5, 9, 'or more as needed',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(24, 24, 5, 9, 'My client',
                   xlwt.Style.easyxf(style_ordinary))

    ws.write_merge(26, 26, 1, 3, 'Position',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(26, 26, 5, 7, f'{employee_hiring_details.position}',
                   xlwt.Style.easyxf('borders: top_color white, bottom_color black, right_color white, left_color white,\
                              left thin, right thin, top thin, bottom thin;'))
    ws.write_merge(28, 28, 1, 3, 'Salary',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(28, 28, 5, 6, f'{employee.salary_per_day}',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(28, 28, 7, 8, f' Everyday',
                   xlwt.Style.easyxf('borders: top_color white, bottom_color black, right_color white, left_color white,\
                              left thin, right thin, top thin, bottom thin;'))
    ws.write_merge(30, 30, 5, 6, 'and will be paid',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(30, 30, 7, 9, f'{employee.salary_per_month} Every month',
                   xlwt.Style.easyxf('borders: top_color white, bottom_color black, right_color white, left_color white,\
                              left thin, right thin, top thin, bottom thin;'))

    ws.write_merge(32, 32, 0, 9, 'Also, we retain the right to termminate your employement unconditionally and',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(33, 33, 0, 9, 'You are not responsible for your departure at any time, when or',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(34, 34, 0, 9, 'any of the following occurred:',
                   xlwt.Style.easyxf(style_ordinary))

    ws.write_merge(36, 36, 1, 9, '1. When your service is no longer needed due to lack of materials in',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(37, 37, 1, 9, 'working or corresponding buyer or customer.',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(38, 38, 1, 9, '2. When your service is unsatisfactory or your qualification is lacking in your',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(39, 39, 1, 9, 'work.',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(40, 40, 1, 9, '3. When you violate any of our company guidelines.',
                   xlwt.Style.easyxf(style_ordinary))

    ws.write_merge(43, 43, 6, 9, 'I read and understood and I am',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(44, 44, 6, 9, 'in line with the above',
                   xlwt.Style.easyxf(style_ordinary))
    ws.write_merge(45, 45, 6, 9, 'condition about my entrance',
                   xlwt.Style.easyxf(style_ordinary))
